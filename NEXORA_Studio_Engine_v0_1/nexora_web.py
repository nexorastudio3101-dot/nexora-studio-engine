...
def page():
 ...
 if st['error']: msg=f'<div class="error">{html.escape(st["error"])}</div>'
 ...
 return h+f'''<body><main class="wrap"><div class="brand">NEXORA STUDIO ENGINE</div><div class="tag">SMARTER TODAY. BRIGHTER TOMORROW.</div><div class="modes"><div class="mode active">ACADEMY</div><div class="mode">YOUTUBE</div><div class="mode">SHORTS</div></div><form method="post" enctype="multipart/form-data" action="/generate"><div class="card"><div class="title">VOICE / AUDIO</div><div class="hint">Select your narration audio</div><input name="audio" type="file" accept="audio/*" required></div><div class="card"><div class="title">SCRIPT</div><div class="hint">Paste the narration directly — no .txt file needed</div><textarea name="script" required placeholder="Paste your full narration here..."></textarea></div><button type="submit" {'disabled' if disabled else ''}>✨ &nbsp; GENERATE ACADEMY VIDEO</button></form><div class="status">{html.escape(st['status'])}</div><div class="sub">{html.escape(st.get('stage',''))}</div><div class="bar"><div class="fill"></div></div>{msg}<div class="foot">ACADEMY • 16:9 • NEXORA VISUAL SYSTEM</div></main><script>setInterval(()=>fetch('/status').then(r=>r.json()).then(s=>{if(s.status!=='Ready'||s.output||s.error) location.reload()}),1200)</script></body></html>'''
